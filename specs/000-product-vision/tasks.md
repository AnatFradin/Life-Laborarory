...
- [X] T120 [US6] Add "Talk to [Persona Name] in ChatGPT" button to ComposeView in frontend/src/views/ComposeView.vue: appears when reflection has content, opens new tab with generated ChatGPT link including reflection + persona prompt
- [X] T121 [P] [US6] Create ExternalAIDialog component in frontend/src/components/ExternalAIDialog.vue using Radix Vue Dialog: explains external AI flow ("Your reflection will open in ChatGPT..."), has textarea labeled "Paste AI Response Summary" for user to copy back conversation insights, Save/Cancel buttons, keyboard accessible
- [X] T122 [US6] Update useReflections composable in frontend/src/composables/useReflections.js: add saveExternalAIResponse(reflectionId, personaId, summary) method to attach pasted summary + metadata to reflection
- [X] T123 [US6] Update ReflectionList component in frontend/src/components/ReflectionList.vue: show small persona icon badge (emoji) for reflections with externalAISession, add tooltip with persona name on hover
...
- [X] T088 [P] [US2] Add image validation in backend/src/adapters/http/middleware/validation.js: check MIME type (jpeg, png, gif, webp, pdf), max size 10MB per data-model.md (implemented in reflections.js route with multer limits and MIME type checking)
...
- [X] T096 [US2] Ensure visual reflections receive equal design attention: same calm styling, timestamps, accessibility as text mode per FR-003 and SC-014
